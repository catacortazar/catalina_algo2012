...
class vectorField {
    ...
    public:
		...
		// the internal dimensions of the field:    (ie, 60x40, etc) - cuantas flechas vamos hacer - no una por pixel sino una trama con menos 
        int fieldWidth;
        int fieldHeight;
		int fieldSize;   // total number of "pixels", ie w * h
	
		// the external dimensions of the field:   (ie, 1024x768) la pantalla
		int externalWidth;
		int externalHeight;
        ...
        virtual ~vectorField();  //busca en la extension de la funcion y ve si hay algo re-define ahi
